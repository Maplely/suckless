...
#define STR_BUF_MAX 262144 /* 256KB cap for OSC/DCS string buffers */
...
base64dec(const char *src)
{
	...
	if (alloc < in_len) /* overflow check */
		return NULL;
	result = dst = xmalloc(alloc);
	while (*src) {
...
getsel(void)
{
	...
	bufsize = (size_t)(term.col+1) * (sel.ne.y-sel.nb.y+1) * UTF_SIZ;
...
stty(char **args)
{
	...
	*q = '\0';
	...
	if (stty_pid == 0) {
		...
		perror("Couldn't fork for stty");
	} else {
		waitpid(stty_pid, NULL, 0);
	}
}
...
tputc(Rune u)
{
	...
	if (term.esc & ESC_STR) {
		...
		if (strescseq.len+len >= strescseq.siz) {
			 ...
			 * strhandle();
			 */
			if (strescseq.siz > STR_BUF_MAX - (strescseq.siz / 2))
				return;
			strescseq.siz = MIN(strescseq.siz * 2, STR_BUF_MAX);
