...
createmon(void)
{
	...
	for (j = 0; j < LENGTH(monrules); j++) {
		...
		if ((mr->monitor == -1 || mr->monitor == m->num)
			...
			strncpy(m->ltsymbol, layouts[layout].symbol, sizeof m->ltsymbol - 1);
			m->ltsymbol[sizeof m->ltsymbol - 1] = '\0';
...
run(void)
{
	...
	/* main event loop */
	while (running) {
		if (sig_quit_requested) {
			sig_quit_requested = 0;
			Arg a = {.i = sig_quit_type};
			quit(&a);
			if (!running)
				break;
		}
...
main(int argc, char *argv[])
{
	...
	for (int i=1;i<argc;i+=1)
		...
		{
			#if BAR_PANGO_PATCH
			size_t flen = strlen(argv[i + 1]);
			if (flen < sizeof(font))
				memcpy(font, argv[++i], flen + 1);
			else
