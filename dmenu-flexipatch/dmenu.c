...
printitem(struct item *item)
{
	...
	if (item->text[0] == startpipe[0]) {
		...
		size_t plen = sizeof(pipeout) - 1;
		char *p = realloc(item->text, n + plen + 1);
		if (!p)
			die("realloc:");
		item->text = p;
		memcpy(item->text + n, pipeout, plen);
		item->text[n + plen] = '\0';
		puts(item->text + 1);
		return;
	}
...
printtext(char *text)
{
	...
	if (text[0] == startpipe[0]) {
		...
		size_t plen = sizeof(pipeout) - 1;
		if (n + plen + 1 > sizeof text)
			return;
		memcpy(text + n, pipeout, plen);
		text[n + plen] = '\0';
		puts(text + 1);
		return;
	}
